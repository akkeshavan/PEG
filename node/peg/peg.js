var pegToken= require("./pegtoken.js");
 ...
 //Helper functions
	...
	var entryPoint= function (parser){
		return function (str){
			var result=parser(str);
			if(isError(result)){
				var err=tokenValue(result);
			    var errStr= str.substring(0,str.length-err.length)+ "^^"+err;
...
module.exports={
		...
		//not often used 
		emptyAbs:emptyAbs,    //empty() without skipping leading white spaces
		matcherAbs:matcherAbs // matcher()w ithout skipping leading whitespaces.
	} 
